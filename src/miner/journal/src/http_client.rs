...
impl JournalIterator {
    ...
    pub async fn next_page(&mut self) -> DmcResult<Vec<JournalLog>> {
        ...
        let url = Url::parse_with_params(&format!("{}journal", self.client.config().endpoint), to_url_params(params))?;
...
impl std::fmt::Display for JournalClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "UserJournalClient{{endpoint={}}}", self.config().endpoint)
    }
}
...
impl JournalClient {
    ...
    pub async fn append(&self, event: JournalEvent) -> DmcResult<JournalLog> {
        let url = Url::parse(&format!("{}journal", self.config().endpoint))?;
