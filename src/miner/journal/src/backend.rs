...
impl std::fmt::Display for JournalServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "MinerJournalServer")
    }
}

impl JournalServer {
    pub async fn listen(self) -> DmcResult<()> {
        let host = self.config().listen_address.clone();
...
impl JournalServer {
    ...
    fn sql_pool(&self) -> &Pool<sqlx::MySql> {
        &self.0.sql_pool
    }
...
impl JournalServer {
    pub async fn append(&self, event: JournalEvent) -> DmcResult<JournalLog> {
        ...
        // order still waiting 
        let result = sqlx::query("INSERT INTO journal_log (sector_id, order_id, event_type, event_params) VALUES (?, ?, ?, ?)")
            .bind(event.sector_id).bind(event.order_id).bind(event.event_type as u16).bind(event.event_params.as_ref())
            .execute(self.sql_pool()).await.map_err(|err| dmc_err!(DmcErrorCode::Failed, "{} append, event={:?}, update_at={}, err={}", self, event, update_at, err))?;

        let log: JournalLog = sqlx::query_as::<_, JournalLogRow>("SELECT * FROM journal_log WHERE log_id=?").bind(result.last_insert_id())
