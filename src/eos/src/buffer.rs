...
fn decimal_to_binary(size: u8, s: &str) -> DmcResult<Vec<u8>> {
    let mut result = Vec::<u8>::new();
    result.resize(size as usize, 0);
    for (_i, src_digit) in s.chars().enumerate() {
        if src_digit < '0' || src_digit > '9' {
            return Err(dmc_err!(DmcErrorCode::Failed, "invalid number {}", s));
        }
        ...
        if carry != 0 {
            return Err(dmc_err!(DmcErrorCode::Failed, "number is out of range"));
        }
    }
...
fn signed_decimal_to_binary(size: u8, s: &str) -> DmcResult<Vec<u8>> {
...
pub struct EosSerialRead<'a> {
    array: &'a [u8],
    read_pos: usize,
    length: usize,
}

impl<'a> EosSerialRead<'a> {
    ...
    pub fn get(&mut self) -> DmcResult<&u8> {
        if self.read_pos < self.length {
            let ret = self.array.get(self.read_pos);
            self.read_pos += 1;
            Ok(ret.unwrap())
        } else {
            Err(dmc_err!(DmcErrorCode::Failed, "read_pos {} length {}", self.read_pos, self.length))
        }
    }

    pub fn get_u8(&mut self) -> DmcResult<u8> {
        self.get().map(|i| *i)
    }

    pub fn get_array(&mut self, len: usize) -> DmcResult<&[u8]> {
        if self.read_pos + len > self.length {
            Err(dmc_err!(DmcErrorCode::Failed, "read_pos {} length {}", self.read_pos + len, self.length))
        } else {
    ...
    pub fn get_u16(&mut self) -> DmcResult<u16> {
    ...
    pub fn get_u32(&mut self) -> DmcResult<u32> {
    ...
    pub fn get_u64(&mut self) -> DmcResult<u64> {
    ...
    pub fn get_var_u32(&mut self) -> DmcResult<u32> {
        let mut v = 0;
        let mut bit = 0;
        loop {
            let b = self.get_u8()?;
    ...
    pub fn get_var_int32(&mut self) -> DmcResult<i32> {
    ...
    pub fn get_f32(&mut self) -> DmcResult<f32> {
        if self.read_pos + 4 > self.length {
            Err(dmc_err!(DmcErrorCode::Failed, "read_pos {} length {}", self.read_pos, self.length))
        } else {
            let mut b = [0u8; 4];
    ...
    pub fn get_f64(&mut self) -> DmcResult<f64> {
        if self.read_pos + 8 > self.length {
            Err(dmc_err!(DmcErrorCode::Failed, "read_pos {} length {}", self.read_pos, self.length))
        } else {
            let mut b = [0u8; 8];
    ...
    pub fn get_name(&mut self) -> DmcResult<String> {
    ...
    pub fn get_bytes(&mut self) -> DmcResult<&[u8]> {
        let len = self.get_var_u32()?;
        self.get_array(len as usize)
    }

    pub fn get_string(&mut self) -> DmcResult<String> {
        let len = self.get_var_u32()?;
        let array = self.get_array(len as usize)?;
        Ok(String::from_utf8_lossy(array).to_string())
    }

    pub fn get_symbol_code(&mut self) -> DmcResult<String> {
        let a = self.get_array(8)?;
        let mut i = 0;
        while i < a.len() && a[i] != 0 {
            i += 1;
    ...
    pub fn get_symbol(&mut self) -> DmcResult<(String, u8)> {
    ...
    pub fn get_asset(&mut self) -> DmcResult<String> {
    ...
    pub fn get_public_key(&mut self) -> DmcResult<String> {
        ...
        if key_type == 0 || key_type == 1{
            ...
            if key.is_err() {
                return Err(dmc_err!(DmcErrorCode::Failed, "parse public key failed"));
            }

            let public_key = EosPublicKey {
                key_type,
                key: key.unwrap()
            };
            ...
            return Err(dmc_err!(DmcErrorCode::NotSupport, "not support"));
        } else {
            return Err(dmc_err!(DmcErrorCode::NotSupport, "not support"));
        }
    }

    pub fn get_private_key(&mut self) -> DmcResult<String> {
        let key_type = self.get()?;
        let key_type = match *key_type {
            ...
            2 => EosKeyType::Wa,
            v @ _ => {
                return Err(dmc_err!(DmcErrorCode::Failed, "unsupport {} key type", v));
            }
        };
        ...
        let key = libsecp256k1::SecretKey::parse_slice(key).map_err(|e| {
            dmc_err!(DmcErrorCode::Failed, "parse key err {}", e)
        })?;

        let private_key = EosPrivateKey {
            key_type,
            key
        };
...
impl Into<Vec<u8>> for EosSerialWrite {
    fn into(self) -> Vec<u8> {
        self.array
    }
}
...
impl EosSerialWrite {
    ...
    pub fn push_name(&mut self, s: &str) -> DmcResult<()> {
        let regex = Regex::new(r#"^[.1-5a-z]{0,12}[.1-5a-j]?$"#).unwrap();
        if !regex.is_match(s) {
            return Err(dmc_err!(DmcErrorCode::Failed, "Name should be less than 13 characters, or less than 14 if last character is between 1-5 or a-j, and only contain the following symbols .12345abcdefghijklmnopqrstuvwxyz"))
        }
    ...
    pub fn push_symbol(&mut self, name: &str, precision: u8) -> DmcResult<()> {
        let regex = Regex::new(r#"^[A-Z]{1,7}$"#).unwrap();
        if !regex.is_match(name) {
            return Err(dmc_err!(DmcErrorCode::Failed, "Expected symbol to be A-Z and between one and seven characters"));
        }
    ...
    pub fn push_asset(&mut self, s: &str) -> DmcResult<()> {
        let s = s.trim();
        let mut pos = 0;
        ...
        if !found_digit {
            return Err(dmc_err!(DmcErrorCode::Failed, "Asset must begin with a number"));
        }
    ...
    pub fn push_public_key(&mut self, s: &str) -> DmcResult<()> {
        let public_key = EosPublicKey::from_str(s)?;
        self.push_u8(public_key.key_type as u8);
        self.push_array(public_key.key.serialize_compressed().as_slice());
        Ok(())
    }

    pub fn push_private_key(&mut self, s: &str) -> DmcResult<()> {
        let private_key = EosPrivateKey::from_str(s)?;
